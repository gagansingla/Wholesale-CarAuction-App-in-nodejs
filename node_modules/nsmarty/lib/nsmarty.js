...
nsmarty.assigndata = function (templateName, params,callback) {
  var stream = new Stream();
  var parsed = nsmarty.cache[templateName];
  
  if (parsed) {
    beginRenderWithOutStream(parsed[0].tokens, params, nsmarty.cache, stream,callback);
  } else {
    nsmarty.compile(templateName, params, function (err, parsed) {
      if (err) return stream.emit('error', err);
      beginRenderWithOutStream(parsed[0].tokens, params, nsmarty.cache, stream,callback);
...
function beginRenderWithOutStream(tokens, params, partials, stream,callback) {
  var count = 0;
  
  process.nextTick(function () {
    try {
       
      renderer.render(tokens, params, partials, stream, function (row) {
        var cnf  = $arr['config'];
        delete $arr;
        $arr = [];
        $arr['config'] = cnf;
